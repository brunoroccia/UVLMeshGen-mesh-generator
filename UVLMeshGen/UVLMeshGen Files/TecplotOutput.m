...
%% *******************************************************************************************************************
%                                A S S E M B L I N G   W I N D   T U R B I N E
%*********************************************************************************************************************

NameTecplot = fullfile (Folders{4,1}, strcat(OutputName,'-Assembly.tec') );
...
%-----------------------------------------------------------------------------------------------------------------------
        ...
        if DATA(i).Nacelle == 1
            ...
            for j = 1:4
                ...
                fprintf(FID, 'ZONE T = "WT %i - Part %i - Nacelle", N = %i , E = %i , F = FEPOINT, ET = QUADRILATERAL, C = BLUE \n', i, j, size(WIND_TURBINE(i).NACPART(j).XYZ,1), size(CONNECT(i).NACPART(j).ICON,1));
                fprintf( FID, '\n');

                for k = 1:size(WIND_TURBINE(i).NACPART(j).XYZ,1)
                    fprintf(FID, '%12.8f\t  %12.8f\t  %12.8f\n', WIND_TURBINE(i).NACPART(j).XYZ(k,1:3));
                end
...
%-----------------------------------------------------------------------------------------------------------------------
        ...
        if DATA(i).Hub == 1
            ...
            for j = 1:length(WIND_TURBINE(i).HUBPART)
                ...
                fprintf(FID, 'ZONE T = "WT %i - Part %i - Hub", N = %i , E = %i , F = FEPOINT, ET = QUADRILATERAL, C = BLUE \n', i, j, size(WIND_TURBINE(i).HUBPART(j).XYZ,1), size(CONNECT(i).HUBPART(j).ICON,1));
                fprintf( FID, '\n');

                for k = 1:size(WIND_TURBINE(i).HUBPART(j).XYZ,1)
                    fprintf(FID, '%12.8f\t  %12.8f\t  %12.8f\n', WIND_TURBINE(i).HUBPART(j).XYZ(k,1:3));
                end
...
%-----------------------------------------------------------------------------------------------------------------------
        ...
        if DATA(i).Blade == 1
            ...
            for j = 1:length(WIND_TURBINE(i).BLDPART)
                ...
                for k = 1:size(WIND_TURBINE(i).BLDPART(j).XYZ,1)
                    fprintf(FID, '%12.8f\t  %12.8f\t  %12.8f\n', WIND_TURBINE(i).BLDPART(j).XYZ(k,1:3));
                end
